...
| 32 | (lihat commit) | **«Designnya rame banget» → tombol `!` + popup.** Semua tata cara di halaman siswa dipindah ke tombol `!` kecil yang membuka popup singkat (`petunjuk()` di `_macros.html`, `.pl-info*` di `portal.css`, pembuka/penutup di `app.js`: satu popup sekaligus, klik luar/Esc menutup). Beranda dipangkas jadi empat blok (sapaan · Perlu dilakukan · kelengkapan satu baris · kegiatan), «Dataku» kehilangan kalimat penjelas per kelompok, «Kegiatan» memakai kartu + riwayat terlipat, halaman masuk kehilangan daftar langkah 1-2-3 dan kotak bantuan panjang, dan footer jadi satu baris. Hero tidak lagi memotong popup (`overflow: visible`). `cek_sistem` blok 16 memeriksa makro `petunjuk`, gaya popup, pembuka di `app.js`, dan bahwa halaman siswa memakainya; blok 22 mengikuti label baru. |
...
*Terakhir diperbarui: ronde 32. Tambahkan kesalahpahaman baru ke §1 begitu
