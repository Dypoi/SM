...
### 27. Keluhan yang sama untuk ke-3 kalinya: yang dibuka sekolah masih salinan lama (ronde 41)

| | | |
|---|---|---|
| **Gejala asli** | Setelah ronde 40 dilaporkan & dikirim, keluhan yang sama datang lagi: «ketika login sebagai pelatih, halamannya saya lihat masih tidak responsi ni». |
| **Yang saya pikirkan** | Berarti masih ada permukaan yang belum diuji — mungkin halaman **login** (belum pernah masuk sapu) atau **HP mendatar**. |
| **Yang sebenarnya** | Dua-duanya sudah diuji dan **bersih**: halaman login 3 panel × 8 lebar (280–1024, termasuk 640×360 & 844×390) nol temuan; halaman pelatih & pembina 11 lebar × 5 keadaan = 55 kombinasi nol temuan. Lalu saya membandingkan **versi r38 (terakhir yang mungkin ada di PC sekolah) vs versi sekarang**: di 390 px versi lama punya **22 elemen keluar tepi kartu** dan di 300 px **65 elemen**; kisi `.form-cari-cepat` tetap `200px 190px 134px 0px` sehingga tombol «Cari siswa» **tergenjet 0 px** (tidak bisa ditekan) dan isi tabel 323 px di dalam ruang 271 px membuat kolom «Keputusan/Aksi» tersembunyi. Versi sekarang: **0** elemen keluar, isian selebar kartu (335 px), tabel 271 = 271 px. Jadi halaman pelatih memang **pernah** tidak responsif — persis seperti yang dilihat sekolah — tetapi itu **build lama**. |
| **Aturan** | Bila keluhan yang sama datang lagi padahal gerbang sudah hijau: (a) **jangan** mengulang jawaban lama — buktikan dulu dengan **membandingkan dua versi** pada lebar & potongan identik; (b) pakai **data realistis** (nama panjang, catatan panjang, banyak anggota) supaya perbedaannya terlihat, bukan halaman kosong; (c) sebutkan **penanda versi** (`APP_VERSION` di footer) sebagai cara memastikan dalam 5 detik; (d) berikan **langkah konkret** memperbarui PC sekolah (menu Pembaruan → Periksa → Tarik → Muat ulang → muat ulang keras di HP). |
| **Pelajaran uji** | (i) Pemeriksa luapan wajib menyaring **laci menu yang memang digeser keluar layar** (`position: fixed/absolute` + `left` negatif) dan **tautan pintasan** (`.skip-link`) — tanpa itu 35 "temuan" palsu muncul dan menutupi hasil sebenarnya. (ii) Kelas `position: static` di dalam laci juga ikut melapor; saring dengan `closest('.sidebar, .sidebar-backdrop, .skip-link')`. (iii) Menyimpan salinan versi lama lewat `git checkout <commit> -- app/static/css app/templates` (lalu `git checkout HEAD -- …` untuk memulihkan) jauh lebih aman daripada `git stash` berulang. |

### 26. «Masih tidak responsif» — sapu berhenti di 360 px, tiga keluarga sebab memotong isi di ≤330 px (ronde 40)

| | | |
|---|---|---|
| **Gejala asli** | Keluhan yang sama diulang *persis* setelah ronde 39 dilaporkan: «ketika login sebagai pelatih, halamannya masih tidak responsif… sisi hp harus bisa responsif tidak boleh ada terpotong». |
| **Yang saya pikirkan** | Halaman sudah bersih: sapu 20 halaman × 11 lebar (360–1568 px) nol temuan, dan gerbang 35/35 hijau. Berarti ini soal versi — pengguna melihat salinan lama. |
| **Yang sebenarnya** | Dua-duanya benar, tetapi yang kedua **belum saya uji**: sapu ronde 39 berhenti di **360 px**. Setelah diturunkan ke 280/300/320/340/360 px, ada **tiga keluarga sebab** yang semuanya memotong isi: (a) **`min-width` tetap di kepala halaman** — enam templat memakai `style="min-width:220-300px"` pada blok judul di dalam kartu (ekskul/detail 260, import/detail 260, approval/detail 240, students/detail 220, bot_dapodik 300, portal/home 170). Terukur di 300 px: ruang dalam kartu hanya **236 px** sehingga kotak 260 px meluber **24 px** — di 280 px nama «Dra. Hj. SITI NURHALIZA…» **terpotong di tengah huruf** karena `.card` memakai `overflow: hidden`. (b) **Kisi `auto-fit` tanpa `min()`** — `minmax(Npx, 1fr)` tidak bisa menyusut di bawah N; di halaman Bot Dapodik kolom `.opsi-bot` 280 px berada dalam ruang **254 px** (keluar 26 px), dan masih ada delapan kisi lain (`.filter-bar` 160, `.grid-2/3/4` 320/240/200, `.form-grid` 230, `.switch` 128, `.form-bot` 220, `.bot-ringkas` 190, `.pl-petak` 215/250). (c) **Teks `nowrap` yang tak boleh turun baris** — tombol «Tambah ekstrakurikuler» terukur **242 px di dalam kotak 216 px** (menembus tepi kartu di 280 px), label diagram batang ber-elipsis jadi «PADUAN S…», `.alert` tanpa `flex-wrap` mendorong tombolnya keluar **+19 px**, dan `.topbar-actions` meluber **2 px** sehingga tiga halaman bisa digeser ke samping di 320 px. |
| **Aturan** | (a) `min-width` tetap di templat **dilarang** — pakai `min-width: min(Npx, 100%)` (nilai <120 px dan sel `<td>/<th>` dikecualikan karena tidak berbahaya). (b) Setiap `minmax(Npx, 1fr)` **wajib** punya pasangan `minmax(min(Npx, 100%), 1fr)` **dengan N sama** dan ditulis **sesudahnya**. (c) Di ≤380 px tombol (`.btn` termasuk `.form-tambah-anggota .btn`) boleh turun baris; label diagram batang membungkus (elipsis terbaca «terpotong» di HP); `.alert` boleh membungkus; `.topbar-title` menyusut (`min-width: 0`) dan `.topbar-actions` tetap utuh. (d) Penanda versi (`APP_VERSION` → footer «SM v0.1.1») dinaikkan tiap ronde perbaikan supaya pengguna bisa **memastikan** versi mana yang sedang dibuka — keluhan «masih» yang diulang sering kali memang soal salinan lama. |
| **Pelajaran uji** | (i) Sapu lebar **wajib turun sampai 280 px** (layar luar Galaxy Fold & panel pratinjau), bukan berhenti di 360 px — justru di rentang itulah ketiga keluarga sebab muncul. (ii) Pemeriksa harus **membuang** dua jenis temuan palsu: `text-overflow: ellipsis` (pemotongan yang disengaja) dan kesalahan tulis di berkas uji sendiri. (iii) Yang menangkap «isi terpotong» bukan `luber kanan` saja, melainkan **wadah ber-`overflow: hidden` dengan `scrollWidth > clientWidth`** — persis di situ `.card` menyembunyikan huruf terakhir. (iv) Bukti sebelum/sesudah yang jujur: `git stash push -m "r40 sementara" -- app/static/css app/templates` → potret keadaan lama → `git stash pop` (cek ulang `grep -c 'min(260px'`). (v) Penjaga baru: blok 36 `cek_sistem.py`. |

### 25. «Halamannya masih tidak responsif» — aturan responsif kalah urutan & tabel yang tak pernah jadi kartu (ronde 39)

| | |
|---|---|
| **Gejala asli** | Login sebagai pelatih, halaman ekstrakurikuler di HP: tombol **«Masukkan ke ekskul» keluar dari kartu**, tabel «Pendaftar dari Siswa» terpotong (kolom «Keputusan» beserta tombol Setujui/Tolak tidak terlihat sama sekali), dan pilihan Jabatan/Nilai di kartu anggota terpotong. *«sisi hp harus bisa responsif tidak boleh ada terpotong»*. |
| **Yang saya pikirkan** | Halaman ini sudah responsif — ada blok `@media (max-width: 560px)` yang menjadikan tiga formulirnya satu kolom, dan tabelnya punya pembungkus bergulir. |
| **Yang sebenarnya** | Dua sebab, dua-duanya **terukur di peramban sungguhan**. (a) **Aturan responsif itu kalah urutan**: aturan satu kolom untuk `.form-tambah-anggota`/`.form-cari-cepat`/`.form-jadwal` ditulis di baris ~759, sedangkan definisi dasarnya (kisi tetap `220px 160px 160px auto`) ada di baris 941–977 — kekhususan sama, **yang belakangan menang**. Terukur di layar 390 px: kisi jadwal tetap `220px 160px 160px 169,4px` dan tombol «Masukkan» berakhir di x = **671 px** padahal kartunya hanya **325 px**. (b) **Tabel baru jadi daftar kartu di ≤ 560 px**, jadi pada 600 px tabel 7–9 kolom (743–1166 px) masuk ke `.table-wrap` yang menggulir diam-diam — kolom terakhir hilang dari pandangan tanpa tanda apa pun. Dari sapu 20 halaman × 11 lebar, penyakit yang sama muncul di enam halaman admin lain (dasbor 474 px, Kualitas Data 671 px, Impor 506 px, Statistik 489 px pada ruang 361 px). |
| **Aturan** | (a) Formulir multi-kolom memakai kisi yang **boleh menyusut sendiri** `repeat(auto-fit, minmax(min(Npx, 100%), 1fr))` — tidak lagi bergantung urutan media query; bila tetap memakai media query, taruh di **akhir berkas** dan tulis alasannya. (b) Tabel: `table.data.kartu` jadi kartu di **≤ 1400 px** (tabel anggota butuh 1166 px, Data Siswa 1097 px; pada 1366 px ruang isi hanya 1112 px), tabel `.data` tanpa kelas `kartu` jadi kartu di **≤ 900 px**. (c) Sel mode kartu wajib `flex-wrap: wrap` dan lencana `white-space: normal` — tanpa itu lencana «1 pendaftar menunggu» (176 px) menembus tepi kartu 361 px. (d) Tabel yang tetap sempit walau layarnya lebar (mis. riwayat bot 407 px di kolom 318 px) diselesaikan dengan **container query**: `.table-wrap { container-type: inline-size }` + `@container (max-width: 760px)`. (e) Halaman daftar ekskul memakai `.split-tabel` supaya kolom sampingnya turun pada 1101–1500 px (kolom kiri 730–865 px < tabel 870 px). |
| **Pelajaran uji** | Yang menemukan semuanya adalah **sapu otomatis**: `scripts/lihat_tampilan.py` memotret & mengukur 20 halaman (admin, pembina, pelatih, siswa) di 11 lebar (360–1568 px) dan melaporkan (i) elemen yang melewati tepi konten, (ii) `.table-wrap` yang `scrollWidth > clientWidth`, (iii) dokumen yang bisa digulir ke samping. Sapu itu juga membongkar **dua kesalahan alat**, bukan kesalahan halaman: **tab peramban tidak pernah ditutup** (`Sesi.tutup()` hanya menutup websocket) sehingga setelah puluhan pemeriksaan memori mesin habis dan pemeriksaan berikutnya gagal «Tidak bisa membuka tab peramban: timed out» — kini tabnya benar-benar ditutup lewat `/json/close/<id>`; dan **CSS yang ditulis lewat skrip harus diperiksa pengurai** — blok `@media` yang dihasilkan sempat terpotong 6 baris (tanpa `}` penutup) dan hanya ketahuan setelah `tinycss2` dipakai. Penjaga baru: blok 35 `cek_sistem.py`. |

### 24. «Kepotong-potong»: kolom kisi tergencet jadi 75 px sehingga nilai isian terpotong (ronde 38)

| | |
|---|---|
| **Gejala asli** | *«jelek kepotong potong gini halam pengajuannya»* + tangkapan layar halaman Ajukan Perubahan: pada kolom kanan isian, tulisan terpotong di tengah kata — «Kec. Ne», «Pagela», «00000», «32041» — dan judul kolom pecah tiga baris («NIPD / NIS Lokal»). |
| **Yang saya pikirkan** | Ronde 37 halaman ini sudah dirapikan dan lulus pemeriksaan (ikon pas, tidak meluber, tidak ada bilah tertutup). |
| **Yang sebenarnya** | Masalahnya bukan pada kartunya, melainkan pada **kisi formulir**: ``repeat(auto-fit, minmax(230px, 1fr))``. Bila wadah tidak cukup untuk **dua** kolom berisi 230 px, peramban tetap menyusun dua kolom lalu kolom kedua **tergencet** — terukur tinggal **74,9 px**, sementara isi isiannya butuh 174 px. Di kerangka siswa (760 px) kartu kiri hanya ~385 px, jadi ini terjadi di hampir semua laptop. Terlihat jelas pada tangkapan layar sekolah (lebar jendela ~1500 px). Dulu tertutup rapi oleh ``.card { overflow: hidden }`` sehingga tidak muncul sebagai luapan. |
| **Aturan** | (a) Kisi berbasis ``auto-fit`` wajib memakai **``minmax(min(230px, 100%), 1fr)``** supaya kolom boleh menyusut dan otomatis jadi satu kolom bila tak muat. (b) Isian di dalam ``.field`` diberi **``min-width: 0``** — isian bawaan peramban punya lebar sendiri (atribut ``size``) dan bisa menolak menyusut. (c) ``.field.span-2`` memakai **``grid-column: 1 / -1``**, bukan ``span 2``, sebab pada kisi satu kolom ``span 2`` menyisipkan kolom kedua yang tersembunyi. (d) Halaman yang memakai dua kolom (isian + lampiran berkas) boleh memakai kerangka lebih lega (``.pl-main-lebar``, 1060 px) — halaman siswa lain tetap 760 px. |
| **Pelajaran uji** | Pemeriksa «tidak meluber» yang lama **tidak menangkap** kasus ini: dokumen tetap selebar layar karena kartu memotong isinya. Alat ukur baru memeriksa **isi kotak terhadap kotaknya sendiri**: ``el.scrollWidth > el.clientWidth`` (teks terpotong) dan kolom lebih sempit dari 130 px. Terbukti tepat: pada aturan lama ia melaporkan «isian «nik» terpotong (kotak 74,9 px, isi butuh 174 px)»; setelah diperbaiki, nol temuan. Alat ini juga sempat **salah tuduh** isian berkas yang disembunyikan (1 px) — penyaringnya ditambah, sesuai pelajaran ronde 35: temuan mentah jangan langsung dipercaya. |

### 23. Halaman «Ajukan Perubahan» terasa aneh: isian berkas berbahasa Inggris & tombol kirim di tengah halaman (ronde 37)

| | |
|---|---|
| **Gejala asli** | *«tinggal bagian halaman /portal/pengajuan ketika login sebagai siswa, yang dimana tampilannya tidak saya sukai, tolong anda fix agar responsif dan tampilannya tidak aneh»*. |
| **Yang saya pikirkan** | Ronde 36 halaman ini sudah lolos pemeriksaan (ikon pas, tidak meluber), jadi mungkin sudah beres. |
| **Yang sebenarnya** | Setelah dipotret dari dekat, ternyata ada empat hal yang memang terasa asing:<br>1. **Isian berkas bawaan peramban** menyisakan tulisan **«Choose File / No file chosen»** — bahasa Inggris, gaya sistem — tiga kali di halaman anak.<br>2. **Tombol «Kirim Pengajuan» muncul di TENGAH halaman**: bilah ``.form-actions.sticky-actions`` lengket di dalam kartu. Di ponsel bilah menu bawah (``z-index: 40``) menang atas bilah tombol (``z-index: 5``) sehingga tombol **tertutup menu**; di PC tombol tampak sebelum bagian lampiran berkas.<br>3. **Lencana «kurang 3»** dan **«8 isian»** pecah dua baris di kolom sempit.<br>4. Pada layar sempit, ``.field.span-2 { grid-column: span 2 }`` membuat kolom kedua tersembunyi sehingga isian jadi setengah lebar. |
| **Aturan** | (a) Isian berkas bawaan peramban **jangan dipakai apa adanya** di halaman siswa: sembunyikan isiannya, sediakan label bergaya tombol, dan tampilkan nama berkas pilihan lewat skrip. (b) Tombol kirim ditaruh **di akhir halaman** (setelah kolom isian *dan* setelah lampiran berkas) — jangan bilah lengket di dalam kartu yang bisa tertutup bilah menu bawah. (c) Label pendek (lencana, «N isian») diberi ``white-space: nowrap``. (d) ``grid-column: span N`` wajib dimatikan di layar satu kolom. |
| **Pelajaran uji** | «Tampilannya tidak saya sukai» sering bukan soal ukuran saja, tapi soal **bahasa & urutan**. Karena itu halaman diperiksa utuh: dipotret penuh, dipotong-potong untuk dibaca teksnya, lalu **alurnya diuji** (memilih berkas → nama muncul; mengirim form → berkas diterima server). Penjaga barunya: blok 33 di ``cek_sistem.py`` (statis + lewat HTTP). |

### 22. Ikon menempel di pojok lencana, kalimat pecah, emoji jadi kotak, kolom meluber di ponsel (ronde 36)

| | |
|---|---|
| **Gejala asli** | *«tinggal bagian login sebagai siswa aja nih, tampilannya coba anda cek lagi apakah sudah pas penempatan icon nya»*. |
| **Yang saya pikirkan** | Ronde 34 sudah merapikan ikon di halaman masuk, jadi tinggal memastikan. Ternyata **tidak**: begitu halaman benar-benar dipotret peramban, terlihat ikon menempel di pojok kotaknya, kalimat pengantar pecah tiga baris, dan emoji jadi kotak kosong. |
| **Yang sebenarnya** | Empat kesalahan terpisah, semuanya karena **satu aturan CSS tentang teks ikut mengenai lencana ikon** atau karena **wadah lupa diizinkan menyusut**:<br>1. `.pl-aksi span { display: block }` (keterangan di kartu) mengenai `span.pl-aksi-ikon` karena kekhususannya lebih tinggi → lencana jadi `block`, ikon 25 px menempel di **pojok kiri-atas** kotak 46 px (terukur **10,5 px** melenceng).<br>2. `.pl-sapa strong { display: block }` mengenai kata **NISN** di tengah kalimat → «Masuk pakai NISN saja ya.» pecah **tiga baris**.<br>3. Emoji `👋`/`👍`/`🙂` bergantung font sistem; di peramban pemeriksa muncul **kotak kosong**.<br>4. Anak grid `.split` bawaannya `min-width: auto` (selebar isi terkecilnya) → di halaman Pengajuan kartu melar jadi **404 px di layar 390 px**; siswa harus menggulir ke samping. |
| **Aturan** | (a) Aturan teks selalu dibatasi (`>` atau `:not(.kelas-ikon)`) supaya tidak mengenai lencana; lencana ikon diberi **selector pelindung** yang lebih khusus (`.pl-aksi .pl-aksi-ikon`). (b) Jangan pakai emoji sebagai ikon di muka pengguna — pakai makro `icon()` yang selalu tergambar sama. (c) Anak grid selalu diberi **`min-width: 0`** supaya kolom boleh menyusut di layar sempit. |
| **Pelajaran uji** | Pertanyaan «sudah pas atau belum» **tidak bisa** dijawab dengan membaca kode maupun dengan mencocokkan teks HTML. Alat baru `scripts/lihat_tampilan.py` mengendalikan peramban sungguhan lewat DevTools Protocol lalu: (1) memotret halaman, (2) **mengukur titik** setiap ikon di dalam wadahnya dan terhadap teks di sebelahnya, (3) memeriksa isi yang tertutup bilah menu bawah, dan (4) memeriksa luapan mendatar. Alat ini juga **diuji balik**: bug lama dikembalikan sebentar — pemeriksanya harus menangkapnya (terbukti «10,5 px naik dan ke kiri»). Pembelajaran ronde 35 terulang: jangan percaya temuan mentah — versi pertama alat ini sempat menuduh ikon kaca pembesar «tidak di tengah» padahal letaknya memang di awal kotak pencarian. |

### 21. Kartu kegiatan meluber karena kelas baris dipakai untuk isi bertumpuk (ronde 34 → 35)

| | |
|---|---|
| **Gejala asli** | Sekolah mengirim tangkapan layar halaman **Kegiatan**: isi kartu tersusun mendatar (`Jadwal belum ditetapkan · 0 anggota` di tengah), tombol **Daftar** menyembul ke luar kartu, dan kartu berikutnya tertimpa. *«tinggal bagian login sebagai siswa aja nih janggal tampilannya»*. |
| **Yang saya pikirkan** | Ronde 34 hanya menambah gaya `.pl-kegiatan` di CSS, jadi susunannya pasti benar. |
| **Yang sebenarnya** | Kartu itu tetap memakai kelas **`.pl-aksi`** di template, dan `.pl-aksi` adalah **baris mendatar** (`display: flex; align-items: center`) untuk tombol/kartu satu-baris (ikon + teks). Isi kartu kegiatan (kepala, jadwal, tombol) adalah **blok bertumpuk**, sehingga flex menaruhnya berdampingan dan isinya meluber ke luar kotak. Aturan `.pl-kegiatan` tidak pernah menang karena `.pl-aksi` sudah menetapkan `display: flex` dengan arah baris, dan `flex-direction` tidak pernah ditulis. |
| **Aturan** | Jangan memakai kelas baris (`.pl-aksi`) untuk wadah yang isinya bertumpuk; kartu bertumpuk memakai kelasnya sendiri dengan **`display: flex; flex-direction: column`** yang eksplisit. Untuk lingkup lebih luas: (a) jangan menaruh **tag blok di dalam tag sebaris** (`<span><div>…`) — peramban memperbaikinya sendiri sehingga letak elemen meleset; (b) wadah baris diberi **`flex-wrap`** dan anaknya **`min-width: 0`** supaya teks panjang membungkus, bukan melimpah keluar kartu. |
| **Pelajaran uji** | Uji «status 200» + «teksnya ada» **tidak bisa** melihat susunan. Karena itu ada alat baru `scripts/cek_tampilan.py` (masuk `cek_sistem` blok 31) yang membaca HTML **hasil render** dan menolak: tag blok di dalam tag sebaris, `.pl-aksi` berisi blok, `.pl-kegiatan` dipakai bersama `.pl-aksi` atau bukan `column`, dan wadah baris tanpa `flex-wrap`/`min-width: 0`. Semua aturan itu berasal dari kesalahan nyata di ronde ini. |
...
| Ronde | Commit | Inti |
|---|---|---|
| 41 | (lihat commit) | **Penelusuran keluhan ke-3: yang dibuka sekolah = salinan lama (r38).** Uji permukaan yang belum pernah disapu: **halaman login 3 panel** × 8 lebar (280–1024, termasuk HP mendatar) → bersih; halaman pelatih & pembina 11 lebar × 5 keadaan (55 kombinasi) → 0 temuan. Perbandingan dua versi dengan data realistis (nama & catatan panjang): versi r38 di 390 px punya **22 elemen keluar tepi kartu** (65 di 300 px), kisi «Cari cepat siswa» tetap `200px 190px 134px 0px` sehingga tombol «Cari siswa» **0 px** (tak bisa ditekan), isi tabel 323 px di ruang 271 px; versi sekarang 0 elemen keluar, isian 335 px, tabel pas. Bukti `pratinjau/lihat/bandingkan.html` + langkah memperbarui PC sekolah lewat menu Pembaruan. Tidak ada perubahan kode; gerbang tetap 36/36 & 57/57. |
| 40 | (lihat commit) | **«Masih tidak responsif» (keluhan diulang) → sapu diturunkan ke 280–360 px.** Tiga keluarga sebab yang semuanya memotong isi di layar sangat sempit: (a) `min-width` tetap 220–300 px di enam kepala halaman (di 300 px kotak 260 px meluber 24 px → huruf terakhir terpotong di dalam kartu); (b) delapan kisi `minmax(Npx, 1fr)` tanpa `min(…, 100%)` (`.opsi-bot` 280 px dalam ruang 254 px); (c) teks `nowrap` yang tak boleh turun baris — tombol 242 px di kotak 216 px, label diagram ber-elipsis, `.alert` mendorong tombol keluar +19 px, dan `.topbar-actions` meluber 2 px sehingga halaman bisa digeser ke samping. Perbaikan: `min-width: min(Npx, 100%)` di seluruh templat, pasangan `minmax(min(Npx, 100%), 1fr)` untuk semua kisi, aturan ≤380 px (tombol boleh turun baris) + label membungkus + bilah atas menyusut. Bukti: 20 halaman × 280/300/320/340/360 px **nol temuan**, dan 390/768/1180/1568 px tetap nol (tidak ada kemunduran); `cek_sistem --http` **36/36** (blok 36 baru), `uji_bodap --cepat` **57/57**. Penanda versi → **0.1.1**. |
| 39 | (lihat commit) | **«Halamannya masih tidak responsif» → 20 halaman diuji di 11 lebar layar.** Dua akar: aturan `@media ≤560 px` untuk tiga formulir halaman ekskul **kalah urutan** oleh definisi dasarnya yang ditulis lebih bawah di berkas (kisi tetap 4 kolom → tombol «Masukkan» berakhir di x=671 px pada kartu 325 px), dan tabel `table.data.kartu` baru jadi daftar kartu di ≤560 px sehingga pada 600 px kolom «Keputusan/Aksi» terpotong tanpa tanda. Perbaikan: kisi lentur `repeat(auto-fit, minmax(min(Npx,100%),1fr))` + aturan sempit di **akhir** berkas, mode kartu tabel `<1400 px` (`table.data` tanpa kelas kartu `≤900 px`), sel kartu `flex-wrap` & lencana boleh turun baris, `.split-tabel` untuk halaman daftar ekskul, `@container (max-width:760px)` untuk tabel di kolom sempit. Alat: `.table-wrap` kini `container-type: inline-size`; `lihat_tampilan.py` **menutup tabnya** (sebelumnya hanya websocket → memori mesin habis & «Tidak bisa membuka tab peramban: timed out»). Bukti: 20 halaman × 11 lebar (360–1568 px) **nol temuan**; `cek_sistem --http` **35/35**, `uji_bodap --cepat` **57/57**. |
...
| 38 | (lihat commit) | **Isian tidak lagi terpotong («kepotong-potong»).** Akar masalahnya kisi formulir ``minmax(230px, 1fr)``: bila wadah tak cukup untuk dua kolom, peramban tetap menyusun dua kolom lalu kolom kedua tergencet tinggal **74,9 px** sehingga nilai isian terpotong («Kec. Ne», «00000», «32041»). Diperbaiki dengan ``minmax(min(230px, 100%), 1fr)`` pada ``.form-grid``/``.grid-2/3/4``, ``min-width: 0`` pada isian, ``.field.span-2 { grid-column: 1 / -1 }``, dan kerangka lebih lega (``.pl-main-lebar`` 1060 px) khusus halaman pengajuan siswa. Alat ukur diperluas: memeriksa **teks terpotong** di dalam kotak isian (``scrollWidth > clientWidth``) dan kolom < 130 px — diuji balik dengan menyuntikkan aturan lama (4 temuan) lalu nol temuan setelah perbaikan. **34/34** & **57/57** lulus. |
| 37 | (lihat commit) | **Halaman Ajukan Perubahan dirapikan.** Isian berkas bawaan peramban («Choose File / No file chosen», bahasa Inggris) diganti **panel unggah** berbahasa Indonesia: isian berkas disembunyikan, siswa menekan tombol «Pilih foto / PDF», dan nama+ukuran berkas pilihan muncul (*«akta.jpg · 240 KB · siap dikirim»*) dengan kartu berubah hijau. Tombol «Kirim Pengajuan» dipindah ke **akhir halaman** (``.pl-kirim``) beserta penjelasan singkat — tidak lagi lengket di dalam kartu yang di ponsel tertutup bilah menu bawah. Lencana «kurang 3» & «8 isian» tidak lagi pecah dua baris (``white-space: nowrap``), dan ``.field.span-2`` kembali satu kolom di layar sempit. Diuji utuh: memilih berkas (nama muncul + panel hijau) dan mengirim form (3 berkas diterima server, kartu berkas jadi «lengkap»). Penjaga baru: blok 33 ``cek_sistem.py``; 33/33 & 57/57 lulus. |
| 36 | (lihat commit) | **Penempatan ikon sisi siswa dipastikan dengan mengukur, bukan menduga.** Halaman masuk & ruang siswa dipotret dan diukur lewat peramban sungguhan (Chromium headless + DevTools Protocol). Ditemukan & dibetulkan: (1) ikon di dalam lencana menempel **10,5 px** ke kiri-atas karena aturan `.pl-aksi span` menimpa `.pl-aksi-ikon` → pelindung selector `.pl-aksi .pl-aksi-ikon`; (2) kalimat «Masuk pakai NISN saja ya.» pecah tiga baris karena `.pl-sapa strong` tanpa pembatas → semua aturan kartu sapa dibatasi `>` dan ikon pengganti memakai lingkaran + ikon SVG `smile`; (3) 4 emoji mentah (👋👍🙂) yang bisa jadi kotak kosong di komputer sekolah → diganti ikon SVG; (4) halaman Pengajuan meluber **404 px di layar 390 px** karena anak grid `.split` tidak boleh menyusut → `.split > * { min-width: 0 }`. Alat baru `scripts/lihat_tampilan.py` (potret + ukur ikon + periksa luapan/tumpang-tindih) dan blok 32 `cek_sistem.py`. 32/32 & 57/57 lulus (payload 109 berkas). |
| 35 | (lihat commit) | **Kartu kegiatan tidak lagi meluber.** Kartu kegiatan siswa (dan halaman masuk) diperbaiki: kelas baris `.pl-aksi` dilepas dari kartu bertumpuk, `.pl-kegiatan` kini `display:flex; flex-direction:column` (kepala → jadwal → tombol), kolom grid diperlebar (`pl-petak-lebar`, 250 px) supaya «Jadwal belum ditetapkan» tidak terpotong-potong, dan wadah baris (`.pl-aksi`, `.pl-mini-baris`) diberi `flex-wrap` + `min-width:0`. Alat baru: `scripts/cek_tampilan.py` (HTML hasil render: tag blok di tag sebaris, `.pl-aksi` berisi blok, kelas bentrok, wadah tanpa pembungkus) → masuk `cek_sistem` blok 31; `scripts/ikon_png.py` merender semua ikon jadi `pratinjau/ikon.png` supaya bentuknya bisa diperiksa dengan mata. 31/31 & 57/57 lulus (payload 108 berkas). |
...
*Terakhir diperbarui: ronde 41. Tambahkan kesalahpahaman baru ke §1 begitu
