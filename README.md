...
| **Isi BIO lewat tombol «Ubah»** (centang pada Pengaturan Bot) | Persis potongan skrip sekolah: sesudah baris siswa dipilih & **sebelum** Data Periodik, bot menekan tombol **«Ubah»** (kelas `x-btn-inner-soft-purple-small`, teks «Ubah»), menunggu **3 detik** seperti skrip itu, lalu mengisi **No. Kartu Keluarga, No. Registrasi Akta Lahir, Alamat (Jalan), RT, RW, Kode Pos, Anak ke-berapa, Nama/NIK/Tahun Lahir/Pendidikan Ayah, dan NIK/Tahun Lahir/Pendidikan Ibu** dengan data siswa dari aplikasi SM — nama kolomnya diambil apa adanya dari skrip sekolah (`no_kk`, `reg_akta_lahir`, `alamat_jalan`, `rt`, `rw`, `kode_pos`, `anak_keberapa`, `nama_ayah`, `nik_ayah`, `tahun_lahir_ayah`, `jenjang_pendidikan_ayah`, `nik_ibu`, `tahun_lahir_ibu`, `jenjang_pendidikan_ibu`). Tiap kolom: **Ctrl+A → ketik**, lalu diberi peristiwa **input → change → blur** (sama seperti Data Periodik). Jendela «Ubah» itu panjang dan punya **area gulirnya sendiri**, jadi setiap kolom **dibawa ke layar lebih dulu** — jendelanya digulir (`scrollIntoView` + `scrollTop` induknya) **dan** halaman tetap dicoba (`scrollBy`), diberi jeda kecil antar percobaan sebelum bot menyimpulkan kolomnya tidak ada. Disimpan dengan tombol **«Simpan»** (`bio_simpan`; berbeda dari «Simpan dan Tutup» milik panel Data Periodik), lalu bot memeriksa **jendelanya benar-benar tertutup** — bila masih terlihat, itu ditulis jujur pada log. Data yang **kosong di SM tidak mengosongkan kolom Dapodik**: kolom itu dilewati dan dicatat (`[bio] No. Registrasi Akta Lahir: data siswa kosong — dilewati (kolom Dapodik tidak dikosongkan).`). **Pada halaman sekolah tombol «Ubah»/«Simpan» ada DUA set** — toolbar halaman *dan* panel «Data Rincian PD» di bawahnya (lihat tangkapan layar sekolah) — sehingga bot **memverifikasi**: tiap kandidat «Ubah» ditekan, lalu dipastikan jendela **«Edit Peserta Didik»** benar-benar terbuka (kalau tidak, kandidat berikutnya dicoba); tombol «Simpan» yang dipakai adalah yang berada **di dalam jendela** itu, bukan milik panel «Data Rincian PD». **Jendela «Ubah» punya area gulirnya sendiri** — menggulir halaman (`window.scrollBy`) tidak menolong. Isi jendela dikembalikan ke atas lebih dulu (skrip sekolah mengisi dari atas ke bawah), lalu bot menggeser **isi jendela 250 px sekali geser** dan **mencari ulang kolomnya setiap kali** — jadi posisi gulirnya selalu tepat, tidak bergantung pada "250 px kebanyakan atau kurang banyak"; halaman ikut digulir hanya sebagai pelengkap bila isi jendela sudah mentok (dan berhenti setelah itu supaya tidak menggantung). **Sebagian kolom itu dropdown (combo Ext JS)** — «Pendidikan», «Pekerjaan», dan «Penghasilan» ayah & ibu (persis tangkapan layar sekolah: daftarnya memuat D1…D4, Informal, Lainnya, Non formal, Paket A…; nama kolom aslinya dari DOM halaman sekolah: `pekerjaan_id_ayah`, `penghasilan_id_ayah`, `pekerjaan_id_ibu`, `penghasilan_id_ibu` — `role="combobox"`, `readonly`, `data-componentid="pekerjaancombo-2560"`). Pada kolom seperti itu **mengetikkan teksnya tidak menyimpan apa pun**: tulisan di layar tampak benar, tetapi nilai model Ext JS tetap kosong (dan kolom wajib bertanda bintang merah ikut menahan penyimpanan). Karena itu bot: **membaca pilihannya dari data komponen Ext JS** (`store` combo: `displayField` → teks, `valueField` → nilainya) sehingga daftar tidak perlu terbuka lebih dulu, lalu tetap mencoba membuka daftarnya (tombol panah → kolomnya → `Ext.getCmp(...).expand()`) supaya pilihannya bisa diklik seperti manual, **mencocokkan** nilai SM dengan daftar Dapodik (sama persis → dirapikan, mis. «SMA / sederajat» dibaca «SMA» → nama lain yang lazim dipakai Dapodik, mis. SLTA/SMU untuk SMA → satu-satunya pilihan yang sepadan), lalu **mengklik pilihan itu** dan memeriksa **nilai yang tersimpan** — di Dapodik `getValue()` combo berisi **id** (mis. `2560`), jadi yang diperiksa adalah **`getRawValue()`** (teks pilihannya). **Perilaku daftar dropdown di Dapodik diperlakukan seperti orang memakainya:** bot **mengklik dropdownnya** (tombol panah), lalu **menunggu datanya muncul** — daftar Ext JS tidak langsung berisi (dibaca berulang sampai ada isinya, dan log menyebut berapa kali harus menunggu) — lalu bila pilihan yang dicari belum tampil, bot **menggulir isi daftar itu 150 px sekali geser dan mencari ulang setiap langkah** (daftar pekerjaan/penghasilan panjang dan punya area gulir sendiri), baru mengklik pilihannya. Pilihan yang belum terlihat **tidak pernah diklik**. Bila kliknya tidak menyimpan apa pun (klik ditelan lapisan Dapodik) atau daftarnya tidak mau terbuka, pilihan itu dipasang lewat **model Ext JS**: `select(record)` — record dicari di `store` komponennya — atau `setValue(<nilai dari valueField>)`, lalu diperiksa ulang. Pilihan yang tidak ada di daftar **tidak ditebak**: log menyebutkan pilihan yang benar-benar terlihat pada daftar itu dan kolomnya dilewati dengan jujur. **Daftar dropdownnya tidak lagi bergantung pada Ext JS.** Di sebagian pemasangan Dapodik, `Ext` tidak bisa dipanggil dari skrip luar (semua jalur `Ext.getCmp`/`store`/`select` gagal) dan kadang tombol panah combonya pun tidak terbaca. Karena itu bot punya tiga jalan tambahan: (1) **menekan tombol ↓ pada kolomnya** — begitulah combo Ext JS dibuka, dan cara ini tidak butuh Ext sama sekali; (2) **elemen daftarnya dicari langsung dari DOM** lewat `aria-owns="…-inputEl …-picker-listEl"` (persis DOM halaman sekolah), `…-picker-listEl`, atau `.x-boundlist` di sekitar kolomnya; (3) keadaan terbukanya dibaca dari `aria-expanded`/elemen daftar yang benar-benar terlihat, bukan hanya `isExpanded()` Ext JS. Pilihan, pengguliran, dan pengklikan dilakukan pada elemen daftar itu — jadi jalur ini tetap bekerja walau Ext mati. **Bila pilihan belum ketemu di bagian yang terlihat, bot tidak langsung menyimpulkan «tidak ada»**: daftarnya digulir dari atas sampai bawah sambil mengumpulkan semua pilihan yang pernah terlihat, lalu dicocokkan sekali lagi; penyusuran berhenti begitu pilihan yang sama namanya ketemu (PC sekolah tidak usah menunggu lama), dan bila memang tidak ada, log menyebut berapa kali daftarnya sudah disusuri. Setelah seluruh daftar terlihat, pilihan yang **sama persis** didahulukan atas nama lain (mis. «SMA» dipilih, bukan «SLTA»). Bila sebuah kolom dropdown tetap tidak berhasil, bot menyimpan **bukti layar saat daftarnya terbuka** (`bio-dropdown-<kolom>-*.png` + `.html`) di folder `data/bot/` — supaya keadaannya bisa diperiksa tanpa menebak. Empat kolom baru ikut diisi dari data siswa SM: **Pekerjaan ayah/ibu** dan **Penghasilan ayah/ibu** (tag `bio_ayah_pekerjaan`, `bio_ayah_penghasilan`, `bio_ibu_pekerjaan`, `bio_ibu_penghasilan`). **Bila wadah jendelanya sendiri tidak terbaca** (versi Dapodik sekolah bisa menamai jendelanya berbeda), bot **tidak berhenti**: kolomnya dilacak lewat **namanya di seluruh halaman** — persis `find_element(By.NAME, …)` skrip sekolah — dan halaman digeser **250 px lalu dicari lagi** setiap kali sampai kolomnya tampil. **Keadaan jendela dilaporkan bot sendiri** pada baris log `[bio-rincian]` (judul jendela, kolom BIO yang terbaca di dalam jendela & di halaman, wadah gulir yang ditemukan beserta `scrollTop`/tingginya, dan jumlah tombol «Simpan» — termasuk yang milik panel «Data Rincian PD»), dan bila sebuah kolom tidak ketemu atau nilainya tidak masuk, bot menyimpan **bukti layar** (`.png` + `.html`) di folder `data/bot/` — jadi keadaannya bisa diperiksa tanpa perlu bertanya dulu. Bila Dapodik menolak ketikan ke kolom (nilai tidak jadi tersimpan — `input.value` langsung memang tidak mengubah model Ext JS), bot **mundur ke `Ext.getCmp(<data-componentid>).setValue(...)`** untuk kolom itu dan **memeriksa ulang** nilainya; lognya menyebutkan jalur mana yang dipakai (`terisi: …` atau `terisi lewat Ext JS: …`). Bila Dapodik sekolah tidak punya tombol «Ubah», langkah ini **dilewati tanpa menggagalkan siswa** — dan karena tombolnya diperiksa cepat, halaman yang memang tak punya tombol itu tidak membuat bot menunggu lama. Semua kunci selectornya bisa ditimpa di «Peta tombol Dapodik»: `bio_ubah`, `bio_jendela`, `bio_no_kk`, `bio_reg_akta`, `bio_alamat`, `bio_rt`, `bio_rw`, `bio_kode_pos`, `bio_anak_ke`, `bio_ayah_nama`, `bio_ayah_nik`, `bio_ayah_tahun`, `bio_ayah_pendidikan`, `bio_ibu_nik`, `bio_ibu_tahun`, `bio_ibu_pendidikan`, `bio_simpan`. |
| **Isi Data Periodik sebelum Registrasi** (centang pada Pengaturan Bot) | Persis skrip sekolah: bot **membawa panel «Data Periodik Peserta Didik» ke layar** lebih dulu dengan `scrollIntoView` — panel ini berada di area gulir sendiri (`/html/body/div[2]/div/div/div[2]/div/div/div/div[4]`), sehingga `window.scrollBy` saja tidak menjangkaunya dan malah menggeser bagian halaman yang tidak perlu; sesudah itu halaman tetap digulir **250 px** (`window.scrollBy(0, 250)`) seperti skrip sekolah, dan bila kolomnya belum ketemu semuanya diulang sampai 4× sebelum bot menyimpulkan kolom itu memang tidak ada. Setelah itu bot mengisi **tinggi badan, berat badan, lingkar kepala**, memilih **«Jarak rumah ke sekolah»** — **≤ 1 km → «kurang dari 1 km»**, **> 1 km → «lebih dari 1 km»** (pilihan div[2], sama seperti skrip sekolah) — dan untuk yang lebih dari 1 km sekaligus mengisi kolom **«Sebutkan (dalam kilometer):»** (`jarak_rumah_ke_sekolah_km`), lalu mengisi **jumlah saudara kandung** pada panel *Data Periodik Peserta Didik* — nilainya dari kolom Tinggi Badan / Berat Badan / Lingkar Kepala / Jumlah Saudara Kandung milik siswa di aplikasi SM — lalu menekan *Simpan dan Tutup*. Langkah ini dijalankan sesudah baris siswa dipilih dan **sebelum** tombol *Registrasi* ditekan. Uji khusus bot (tanpa Chrome) ada di `scripts/uji_bot_dapodik.py` — **23 skenario, 131 pemeriksaan**: DOM sekolah (label sesudah input, kelas `x-form-cb-checked`, kolom km nonaktif→aktif), keadaan hanya-terbaca-lewat-kelas, semua klik ditelan → `Ext.getCmp`, kegagalan total → peringatan jujur, model Ext JS tidak ikut (penanda tak pindah → naik `Ext.getCmp` → km aktif & terisi), XPath meleset → kotak dilacak lewat teks labelnya, baris jarak tidak ada → dilewati jujur + isi panel dilaporkan, **BIO lewat tombol «Ubah» (jendela digulir → 13 kolom terisi → «Simpan», kolom yang datanya kosong dibiarkan)**, **dua set tombol «Ubah»/«Simpan» + gulir di dalam jendela (bot tidak menekan tombol milik panel «Data Rincian PD»)**, **«Ubah» pertama tidak membuka jendela → kandidat berikutnya dicoba**, **posisi gulir jendela menentukan → bot menggulir bertahap sampai semua kolom terisi**, **ketikan ditolak Dapodik → nilai dicoba lewat `Ext.getCmp`**, **wadah jendela tidak terbaca → kolom tetap terisi lewat namanya sambil halaman digeser 250 px (cara skrip sekolah) + log `[bio-rincian]`**, **kolom dropdown → pilihannya diambil dari daftarnya (bukan diketik)**, **pilihan di luar daftar → dilaporkan, tidak ditebak**, **klik pilihan yang ditelan → dipilih lewat `select()`/`setValue()`**, **daftar yang tidak mau terbuka → pilihan dibaca dari data komponen `store`**, **daftar dropdown yang lambat muncul & panjang → ditunggu lalu digulir sampai pilihannya terlihat**, **daftar lambat + klik ditelan → digulir lalu dipilih lewat model Ext JS**, **tanpa Ext & tanpa tombol panah → dropdown dibuka lewat tombol ↓ lalu daftarnya dibaca dari DOM (`aria-owns`) dan disusuri sampai pilihan yang sama persis ketemu**, **tombol «Ubah» tidak ada → BIO dilewati jujur**, jarak ≤ 1 km, dan jarak kosong. Kolom yang tidak ada pada versi Dapodik sekolah, atau data siswa yang kosong, hanya dicatat pada log — siswa tetap didaftarkan. Centang «Isi baris Jarak rumah ke sekolah» dapat dimatikan tersendiri bila sekolah tidak memakainya. Tag `periodik_jarak_kurang`/`periodik_jarak_lebih` dapat ditimpa di «Peta tombol Dapodik» bila tata letak Dapodik berbeda. Bila kolom **Jarak Rumah ke Sekolah (KM)** siswa kosong, tidak ada pilihan yang dicentang — Dapodik menolak centang jarak tanpa keterangan kilometernya. **Langkah ini selalu tercatat** — log memuat baris `[periodik] memilih «Jarak rumah ke sekolah» — data siswa 2 km → pilihan «lebih dari 1 km».` sebelum pemilihannya, lalu hasilnya (`… dicentang: 1 dari 1`). **Struktur DOM Dapodik (terperiksa dari halaman sekolah):** radio jaraknya adalah `input[type=radio][name=jarak_rumah_ke_sekolah]` di dalam `div.x-field`, dengan label `label.x-form-cb-label` (**sesudah** input) dan penanda tercentang berupa kelas `x-form-cb-checked` pada pembungkusnya — input-nya sendiri tidak membawa atribut `checked`. Karena itu kotaknya dilacak dari labelnya lewat wadah `.x-field` / poros `preceding::` (bukan `following::`), keadaan tercentangnya dibaca dari ketiga sumber (properti `checked`, kelas `x-form-cb-checked`, `aria-checked`), dan setiap unsur Ext JS membawa `data-componentid` (atau `for="radiofield-1112-inputEl"` pada labelnya) sehingga `Ext.getCmp(...).setValue(...)` adalah **jalur pamungkas**: inilah yang benar-benar mengubah model Ext JS, bukan hanya tampilan. Bot memeriksa **penanda** itu setiap kali memilih — bila tampilan sudah berubah tetapi penanda `x-form-cb-checked` masih di pilihan lain, artinya nilai Ext JS belum berubah (dan kolom kilometer tetap nonaktif), sehingga bot langsung naik ke `Ext.getCmp`. **Jangan bergantung pada satu XPath.** XPath Dapodik (mis. `td/div[2]/…/span/input`) berbeda antar versi dan bisa meleset; dulu bila itu terjadi bot berhenti dengan «kotak … tidak ada di halaman ini» walaupun pilihannya jelas terlihat guru. Sekarang, bila selector tidak menemukan pilihan yang diminta, **kotaknya dilacak lewat teks pilihannya** — dibaca langsung oleh JavaScript (label `for="…-inputEl"` → kotaknya, atau input di dalam `.x-field` yang sama) — lalu labelnya diklik seperti skrip sekolah. Kandidat yang teksnya terbaca tetapi bertuliskan pilihan lain **dibuang** (jangan sampai bot memilih «kurang dari 1 km» hanya karena selectornya mengembalikan satu radio). Setiap pemilihan juga mencatat satu baris jelas: `[periodik] pilihan jarak «lebih dari 1 km»: kotaknya ditemukan (id komponen radiofield-1112) — dicoba klik labelnya → kotaknya → pembungkusnya → Ext.getCmp.`, dan bila jalur Ext JS juga tidak bisa dipakai bot mencatat alasannya (*id komponen tidak terbaca*). Bila pilihan itu memang tidak ada di halaman, log menyebutkan **apa yang benar-benar terlihat di panel** (label & nama kolomnya), bukan sekadar gagal diam-diam. **Kolom «Sebutkan (dalam kilometer)» nonaktif (`disabled`) sampai pilihan «lebih dari 1 km» benar-benar terpasang** — bot menunggu kolomnya aktif lebih dulu, dan bila pilihannya gagal, kolom itu **dilewati dengan jujur** (bukan diisi paksa lalu dianggap berhasil). **Pilihan radio diperiksa ulang:** Ext JS/Dapodik kadang menelan klik yang dikirim lewat skrip (`arguments[0].click()`) — perintahnya seolah berhasil padahal centangnya tidak berubah. Karena itu bot mengecek keadaan terpilih dari halaman, lalu mencoba berurutan: klik **labelnya** (`label` / `x-form-cb-label` yang bertuliskan «lebih dari 1 km» — **persis cara skrip sekolah yang terbukti berhasil**) → klik sungguhan pada kotaknya → klik **pembungkus** kolomnya (`x-form-cb-wrap-inner`) → urutan tetikus lengkap (mousedown → mouseup → click) lewat skrip. Begitu pula setiap kolom isian (berat badan, kilometer, jumlah saudara kandung): setelah diketik, Ext JS diberi tahu lewat peristiwa **input → change → blur** seperti pada skrip itu, supaya nilainya benar-benar terbaca Dapodik. Bila semuanya belum berhasil, log berkata jujur: *“peringatan: Dapodik belum menandainya terpilih (mungkin perlu diklik manual sekali)”* — bot **tidak** lagi mengaku berhasil seperti sebelumnya. Tag `periodik_panel` (wadah panelnya) dapat ditimpa di «Peta tombol Dapodik» bila tata letak Dapodik berbeda.
